...
export function PNodeFilters({ pnodes, onFilterChange }: PNodeFiltersProps) {
    ...
    // Apply filters - use useEffect instead of useMemo to avoid setState during render
    const filteredPnodes = useMemo(() => {
        ...
        return filtered;
    }, [search, selectedStatus, selectedLocations, selectedVersions, minPerformance, pnodes]);

    // Call onFilterChange in useEffect to avoid setState during render
    useEffect(() => {
        onFilterChange(filteredPnodes);
    }, [filteredPnodes, onFilterChange]);
